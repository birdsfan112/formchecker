...
| Needs Scott | (1) Curate remaining 20 clip URLs in `pipeline/sources.yaml` (~2 hrs). Note each clip's facing direction. (2) Identify the 3-4 exercises whose current SVG pickers are wrong (see Backlog item 4 — drawStyle/drawVariant audit). (3) Consider whether the plank/static-hold case warrants its own preset before curating statics. |
...
4. **SVG picker audit** (flagged 2026-04-20) — 3-4 of 22 exercises have picker silhouettes that don't clearly represent the exercise. Current silhouettes come from `EXERCISE_SVGS` keyed on `(drawStyle, drawVariant)` via `getSvgKey` in `index.html`; there are 7 shared shapes covering all 22 exercises. Scott to identify which 3-4 exercises look wrong; fix is either (a) add a new `drawVariant` + SVG for that case, or (b) commission a small set of bespoke per-exercise SVGs via imagegen skill. Signature-derived picker was tried and reverted 2026-04-20: front-view crouched poses don't read as iconic exercises at 70x62px; iconicity > accuracy for pickers.
